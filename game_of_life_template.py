...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        for x in range(0,width):
            for y in range(0, height):
                self.block_list[(x,y)] = Block(Point(x,y),'red')
                #am i supposed to set each block to a random color? idk
    ...
    def seed(self, block_coords):
        ...
        for point in block_coords:
            self.block_list[point].set_live(self.canvas)
    ...
    def get_block_neighbors(self, block):
        ...
        for xc in range (x-1, x+2):
            for yc in range (y-1, y+2):
                SELF = (xc == x and yc == y)
                NEGATIVE = (xc<0 or yc<0)
                TOO_LARGE = (xc>self.canvas.getWidth()-1 or yc>self.canvas.getHeight()-1)
                OUT_OF_BOUNDS = SELF or NEGATIVE or TOO_LARGE

                if not OUT_OF_BOUNDS and self.block_list.has_key((xc,yc)):
                    anslist.append(self.block_list[(xc,yc)])
    ...
    def setNewStatus(self):
        for block in self.block_list.values():
            ...
            n = 0 #where n is the number of live neighbors
            for neighbor in neighbors:
                if neighbor.is_live():
                    n = n + 1

            if block.is_live() and n > 3 or n < 2:
                block.new_status = 'dead'
            elif not block.is_live() and n == 3:
